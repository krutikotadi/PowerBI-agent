...
(Screenshot coming soon — see Setup below to run it yourself.)
...
Runs entirely on your own machine — no API keys, no cloud calls. Just Ollama running Llama 3.2 locally.



🛠️ Tech Stack

LayerToolUIStreamlitVisualizationPlotlyDatapandas / NumPy (simulated sample dataset)AI/agent logicLangChainLLM runtimeOllama running Llama 3.2, fully local
...
This app needs Ollama installed and running locally.

bash# After installing Ollama, pull the model used by this app:
...
app.py builds a sample sales dataset (regions, products, categories, revenue, cost, profit, targets) and looks at what you typed to figure out which charts make sense — region, product, time trend, financial, HR, marketing, or support.
pbi_agent.py checks whether you're asking for an explanation or actual code (wants_dax()), then sends your question to the right prompt — one for talking through the dashboard in plain terms, the other for writing real, structured DAX.
It all runs through Ollama on your own machine, so there's no API key and nothing leaves your computer.
